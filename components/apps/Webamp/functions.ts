  ...
  availableSkins: [
    ...
    ...(ALLOWS_CORS_IN_WINAMP_SKIN_MUSEUM
      ? [
          {
            get name(): string {
              if (!randomSkinLoading) {
                randomSkinLoading = true;
                ...
                fetch("https://skins.webamp.org/graphql", {
                  ...
                  .then(async (response) => {
                    ...
                    randomSkinUrl = data?.skins?.nodes?.[0]?.download_url || "";
                  })
                  .finally(() => {
                    randomSkinLoading = false;
                  });
              }
            ...
            get url(): string {
              return randomSkinUrl || `${WEBAMP_SKINS_PATH}/base-2.91.wsz`;
            },
          },
        ]
      : []),
